...
describe("handles form validation correctly", () => {
  ...
  it("when fields are not the proper length", async () => {
    ...
    fireEvent.change(passwordInput, { target: { value: "lessthan11" } });
    fireEvent.blur(usernameInput);
    fireEvent.blur(emailInput);
    fireEvent.blur(passwordInput);
  ...
  it("when fields are valid", async () => {
    ...
    fireEvent.change(emailInput, { target: { value: "henrique@email.com" } });
    fireEvent.change(passwordInput, { target: { value: "greaterthan11" } });
    fireEvent.blur(usernameInput);
    fireEvent.blur(emailInput);
    fireEvent.blur(passwordInput);
